...
## 2026-05-24
- Stopwatch sub-spans gain a per-plugin cap override: plugin manifests may declare `stopwatch.max_subs: <N>` in the range `[1, 256]` to lift the default 32-entry cap. Manifests outside that range are rejected at plugin load with a warn-level log and the plugin does not register. Default behaviour unchanged for existing plugins. Hard upper of 256 keeps consumer budgets (DB rows, API responses, log lines) bounded across all plugins. Not a global runtime config on purpose — manifest-as-data keeps the cap predictable per-plugin and visible to anyone reading the manifest.
- Stopwatch retention janitor: a new isolated background loop rolls up raw `job_stopwatch` rows into per-day quartile buckets (`job_stopwatch_rollup_daily`, with `sample_count`, `sum_dur_ns`, `min/max/p50/p90/p99 dur_ns`) and then prunes raw rows past the operator-configured retention TTL. Lives in its own goroutine + package (`internal/stopwatchjanitor`) so a janitor failure cannot stop the dispatcher; the failure signal is the new `janitor_heartbeat` row (read by future doctor checks). Operator config: `telemetry.stopwatch.{retention_days, rollup.{enabled, retention_days}, janitor.{interval, batch_size}}` — all optional, zeros mean defaults (14d / 1h / 5000 rows). Existing instances upgrade via `scripts/migrate-add-job-stopwatch-rollup-table.py` (idempotent, hot-safe). Fresh databases get the tables from `BootstrapSQLite`.
