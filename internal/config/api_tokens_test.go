...
func TestResolveAPITokens_MissingRefFailsClosed(t *testing.T) {
	...
	if _, err := ResolveAPITokens(cfg); err == nil {
...
// The core #94 invariant: a literal token value is a secret outside the vault
// and is rejected outright — no migration warning, no coexistence window.
func TestResolveAPITokens_LiteralIsRejected(t *testing.T) {
	cfg := cfgWithTokens(
	...
	if _, err := ResolveAPITokens(cfg); err == nil {
		t.Fatal("expected error: a literal API token must be rejected (vault-only), got nil")
	}
}
...
func TestResolveAPITokens_BothSourcesIsError(t *testing.T) {
	...
	if _, err := ResolveAPITokens(cfg); err == nil {
		t.Fatal("expected error when a literal token is present alongside secret_ref")
	}
}
...
func TestResolveAPITokens_NoTokensIsClean(t *testing.T) {
	...
	if len(got) != 0 {
		t.Fatalf("expected no resolved tokens, got %d", len(got))
	}
}
