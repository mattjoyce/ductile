...
func (s *Server) handlePipelineTrigger(w http.ResponseWriter, r *http.Request) {
	...
	for i, d := range dispatches {
		...
		if d.Command == "handle" {
			...
			if err != nil {
				s.logger.Error("failed to marshal handle command payload", "error", err)
				s.writeError(w, http.StatusInternalServerError, "failed to marshal handle command payload")
				return
			}
		}
