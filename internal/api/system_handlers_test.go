...
func TestDoctorResultToReport(t *testing.T) {
	...
	if report.Checks[0].Severity != "error" || report.Checks[0].OK {
		t.Errorf("first check = %+v, want severity=error ok=false", report.Checks[0])
	}
	...
	if report.Checks[1].Severity != "warning" {
		t.Errorf("second check severity = %q, want warning", report.Checks[1].Severity)
	}

	// Warnings alone must not flip OK.
	...
	if !okReport.OK {
		t.Errorf("warnings-only result: OK = false, want true")
	}
	if len(okReport.Checks) != 1 {
		t.Fatalf("warnings-only checks len = %d, want 1", len(okReport.Checks))
	}
	if okReport.Checks[0].Severity != "warning" {
		t.Errorf("warnings-only check severity = %q", okReport.Checks[0].Severity)
	}
}
...
func TestHandleSelfcheck_HappyPath(t *testing.T) {
	...
	selfFn := func(_ context.Context) (SystemCheckReport, error) {
		return SystemCheckReport{
			...
			CapturedAt: at,
			Checks: []SystemCheck{
				{Name: "config_discovery", OK: true, Detail: "/etc/ductile"},
				{Name: "db_integrity", OK: false, Detail: "skipped: active gateway holds PID lock"},
			},
		}, nil
	}
	...
	if got.Checks[1].Name != "db_integrity" || got.Checks[1].OK {
		t.Errorf("db_integrity check = %+v", got.Checks[1])
	}
...
func TestHandleSelfcheck_CapturedAtDefaultsToNow(t *testing.T) {
	t.Parallel()
	selfFn := func(context.Context) (SystemCheckReport, error) {
		// Closure leaves CapturedAt zero; handler should backfill.
		return SystemCheckReport{OK: true, Checks: []SystemCheck{{Name: "x", OK: true}}}, nil
	}
	server := setupServerWithSystemFuncs(t, nil, selfFn)
	before := time.Now().UTC()
	req := httptest.NewRequest(http.MethodGet, "/system/selfcheck", nil)
